...
export class CosmosHistory extends History{
    async getHistory<T extends IHistoryParams>(params: T): Promise<IHistoryResponse> {
        ...
        try {
            ...
            if(OTHER_SERVICE_CHAINS.includes(chain)){
                const history = await historyFactory[chain](address, pageNumber, pageSize)

                return { result: history ,}
            }
            const chainData = CHAIN_DATA[chain]

            if(chainData.isCosmos && (chainData.environment === 'devnet' || chainData.environment === 'testnet')){
                return { result: [] }
            }

            const endpoint = ChainRest[chain]

            const receiveApi = `${endpoint}/cosmos/tx/v1beta1/txs?events=coin_received.receiver=%27${address}%27&pagination.offset=${(pageNumber - 1) * pageSize}&pagination.limit=${pageSize}`;
            const sendApi = `${endpoint}/cosmos/tx/v1beta1/txs?events=message.sender=%27${address}%27&pagination.offset=${(pageNumber - 1) * pageSize}&pagination.limit=${pageSize}`;
            ...
            const result: IHistoryObject[] = finalData.map((it): IHistoryObject => {
              ...
              let fromAddress: string = tx?.from_address || get(it, 'tx.body.messages[0].sender') || get(it, 'tx.body.messages[0].contract') || get(it, 'tx.body.messages[0].hash')
              let toAddress: string = tx?.to_address || get(it, 'tx.body.messages[0].contract')  || get(it, 'tx.body.messages[0].hash')
